...
//						ㄴ DAO 클래스는 '객체를 1개만 생성되도록하는 디자인패턴' - 싱글턴으로 만듭니다.
...
public class MemberDao {
	...
	public static MemberDao getMemberDao() {			//생성된 객체를 리턴해주는 public 메소드 - 항상 동일한 객체를 리턴
		return dao;
	}
	...
	public void update(Member member) throws SQLException {
		...
		//conn.rollback();	//하나의 트랜잭션을 구성하는 여러 SQL중 일부 오류일때. 이미 실행된
		conn.close();
	}
	...
	public Member selectOne(int custno) throws SQLException {
		//pk값 custno를 인자로 전달받아서 해당되는 조회결과를 Member객체로 리턴
		Member member = null;
		Connection conn = OracleUtil.getConnection();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
...
//insert, update, delete SQL에 대한 메소드 정의. and 테스트